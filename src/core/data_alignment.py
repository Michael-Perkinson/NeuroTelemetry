...
def extract_and_process_data(
    ...
    video_date_time: str,
    skip_rows:int
) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.DataFrame, pd.DataFrame, datetime]:
    ...
    data, meta_data, numerical_data = split_and_parse_data(data, skip_rows)
    ...
    return pressure_data, pressure_data['Pressure'].copy(), temp_data, activity_data, numerical_data, new_reference_timestamp
...
def split_and_parse_data(
    ...
    try:
        numerical_data['DateTime'] = pd.to_datetime(
            numerical_data['DateTime'], format='%m/%d/%Y %I:%M:%S %p', errors='raise'
        )
    ...
    return data, meta_data, numerical_data
