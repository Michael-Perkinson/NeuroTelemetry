...
def load_pressure_data(
    ...
    try:
        ...
        event_dir_files = format_directory_listing(
...
def run_pressure_pipeline(
    ...
    try:
        ...
        processed_data = process_telemetry_to_aligned_frame(
            telemetry_df,
            behaviour_data,
            video_time,
        ...
        pressure_data = require_signal_frame(processed_data, "Pressure")
        temp_data = require_signal_frame(processed_data, "Temp")
        activity_data = require_signal_frame(processed_data, "Activity")
        atm_pressure_data = require_signal_frame(processed_data, "AtmPressure")
        ...
        time_windows = select_fully_covered_windows(
            behaviour_to_plot,
            processed_data["Behaviours"],
            new_reference_timestamp,
            telemetry_bounds=(min_time, max_time),
        ...
        summary_data = summarize_peak_counts_per_window(
            valid_peak_times_all, updated_valid_periods, time_windows
        )

        log("Creating output folders...")
        html_folder, svg_folder, full_trace_folder, base_name = (
            create_folders_for_graphs(analysis_folder)
        )
        ...
        metadata["FailedAt"] = datetime.now().isoformat()
